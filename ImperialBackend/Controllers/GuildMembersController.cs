...
namespace ImperialBackend.Controllers
{
    ...
    public class GuildMembersController : ControllerBase
    {
        ...
        {
            var existingMember = _context.GuildMembers.FirstOrDefault(m =>
                m.Uuid == guildMember.Uuid
            );
            if (existingMember != null)
                return BadRequest("Guild member with this UUID already exists");
            ...
            {
                var invalidMedalIds = guildMember
                    .Medals.Except(_context.Medals.Select(m => m.MedalId))
                    .ToList();
            ...
            {
                var invalidGameIds = guildMember
                    .Games.Except(_context.Games.Select(g => g.GameId))
                    .ToList();
            ...
            if (guildMember.Medals != null)
            {
                ...
                {
                    newMember.Medals.Add(new GuildMemberMedal { MedalId = medalId });
                }
            }
            ...
            if (guildMember.Games != null)
            {
                ...
                {
                    newMember.Games.Add(new GuildMemberGame { GameId = gameId });
                }
            }
            ...
            return Ok(
                new GuildMemberAdminGetDTO
                {
                    ...
                    Rank = new GenericGetDTO
                    {
                        ...
                        Name =
                            createdMember.Rank != null
                                ? createdMember.Rank.RankName
                                : "Error loading name",
                    },
                    JoinDate = createdMember.JoinDate,
                    Games = createdMember
                        .Games.Where(g => g.Game != null)
                        ...
                        .ToList(),
                    Medals = createdMember
                        .Medals.Where(m => m.Medal != null)
        ...
        public IActionResult Put(int id, [FromBody] GuildMemberPostDTO dto)
        {
            ...
            var existingMedalIds = existingMember.Medals.Select(mm => mm.MedalId).ToList();
            var existingGameIds = existingMember.Games.Select(gg => gg.GameId).ToList();
            ...
            // medals to remove
            var medalsToRemove = existingMember
                .Medals.Where(mm => !targetMedalIds.Contains(mm.MedalId))
                .ToList();
            var gamesToRemove = existingMember
                .Games.Where(gg => !targetGameIds.Contains(gg.GameId))
                .ToList();
            ...
            var medalIdsToAdd = targetMedalIds.Except(existingMedalIds).ToList();

            var gameIdsToAdd = targetGameIds.Except(existingGameIds).ToList();

            foreach (var medalId in medalIdsToAdd)
            {
                existingMember.Medals.Add(
                    new GuildMemberMedal
                    {
            ...
            return Ok(
                new GuildMemberAdminGetDTO
                {
                    ...
                    Rank = new GenericGetDTO
                    {
                        ...
                        Name =
                            existingMember.Rank != null
                                ? existingMember.Rank.RankName
                                : "Error loading name",
                    },
                    JoinDate = existingMember.JoinDate,

                    Games = existingMember
                        .Games.Select(g => new GenericGetDTO
                        {
                            Id = g.GameId,
                            Name = g.Game!.GameName,
                        })
        ...
        public IActionResult GetAllPublic()
        {
            var members = _context
                .GuildMembers.Select(m => new GuildMemberPublicGetDTO
                {
                    Id = m.GuildMemberId,
        ...
        {
            var members = _context
                .GuildMembers.Include(m => m.Games)
        ...
        {
            var members = _context.GuildMembers.Include(m => m.PlayerHistoricalStats).ToList();

            var raidCounts = _context
                .RaidsCompleted.Where(r =>
                    r.CompletedDate >= startDate && r.CompletedDate <= endDate
                )
                .GroupBy(r => r.Uuid)
                .Select(g => new { Uuid = g.Key, Count = g.Count() })
            ...
            foreach (var m in members)
            {
                ...
                statsInRange.Add(
                    new Models.PlayerHistoricalStat
                    {
                ...
                int warsDiff = 0;
                int hoursDiff = 0;
                int raidCount = raidCounts.TryGetValue(m.Uuid, out var count) ? count : 0;
